...
var stockSymbol = document.getElementById("StockSymbol").value; // get symbol form input hidden

// Connection opened. Subscribe to a symbol
socker.addEventListener('open', function (event) {
...
var unsubscribe = function (symbol) {
    ...
    socker.send(JSON.stringify({ 'type': 'unsubscribe', 'symbol': symbol }));
}

// when the page is being closed, unsubscribe from the websocket
window.onunload = function () {
    unsubscribe(stockSymobl);
}